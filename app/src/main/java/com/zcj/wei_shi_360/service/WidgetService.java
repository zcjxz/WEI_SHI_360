        ...
        if (timer==null&&task==null) {
            ...
            task = new TimerTask() {
                ...
                public void run() {
                    ...
                    //设置更新组件
                    ComponentName componentName = new ComponentName(WidgetService.this, MyAppWidget.class);
                    RemoteViews views = new RemoteViews(getPackageName(), R.layout.my_app_widget);
                    views.setTextViewText(R.id.appwidget_runing, "正在运行：" + SystemInfoUntil.getRuningProcessCount(WidgetService.this));
                    long availMem = SystemInfoUntil.getAvailMem(WidgetService.this);
                    views.setTextViewText(R.id.appwidget_availmem, "可用内存：" + Formatter.formatFileSize(WidgetService.this, availMem));
